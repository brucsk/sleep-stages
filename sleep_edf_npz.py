...
# %cd /content/drive/MyDrive/DeepLearning/3A

#datad="/home/allauzen/dev/edf/5-cassette"
datad = "./project/5-cassette"
...
axs[0].plot(x[1, :, 2], color = '#62BCE9')
axs[0].set_title('Plot 1')

axs[1].plot(x[2, :, 2], color = '#62BCE9')
axs[1].set_title('Plot 2')

axs[2].plot(x[3, :, 2], color = '#62BCE9')
axs[2].set_title('Plot 3')

axs[3].plot(x[4, :, 2], color = '#62BCE9')
...
for fn in tqdm(fnames):
    ...
    # Extract the EOG channel data and labels
...
#         self.fc1 = nn.Linear(32 * 300, 1024)
...
# class SleepStageNet(SleepStageNet):
#     def __init__(
#         self,
#         batch_size,
#         input_dims,
...
#       # Adjust input size for RNN
#       # Ensure that x.size(1) matches the expected input size of the LSTM
#       x = x.view(-1, self.seq_length, 32)  # Adjust 32 to the expected input size of the LSTM
...
#     return np.array(y_true), np.array(y_pred), epoch_loss

# # Define the evaluation function for one epoch
# def evaluate_model(model, x_valid, y_valid, criterion, device):
#     model.eval()
#     running_loss = 0.0
#     y_true = []
#     y_pred = []
...
# # Define the training function for the entire model
# def train_model(model, x_train, y_train, x_valid, y_valid, criterion, optimizer, device, n_epochs, output_dir):
#     best_valid_loss = float('inf')
#     for epoch in range(n_epochs):
#         # Training
#         train_y_true, train_y_pred, train_loss = train_epoch(model, x_train, y_train, criterion, optimizer, device)

#         # Validation
#         valid_y_true, valid_y_pred, valid_loss = evaluate_model(model, x_valid, y_valid, criterion, device)

#         # Compute metrics
#         train_acc = np.mean(train_y_true == train_y_pred)
#         train_f1 = f1_score(train_y_true, train_y_pred, average='macro')
#         valid_acc = np.mean(valid_y_true == valid_y_pred)
#         valid_f1 = f1_score(valid_y_true, valid_y_pred, average='macro')

#         # Print performance
#         print_performance(train_y_true, train_y_pred, train_loss, train_acc, train_f1,
#                           valid_y_true, valid_y_pred, valid_loss, valid_acc, valid_f1,
#                           epoch, n_epochs)
...
# # Define the function to print performance
# def print_performance(train_y_true, train_y_pred, train_loss, train_acc, train_f1,
#                       valid_y_true, valid_y_pred, valid_loss, valid_acc, valid_f1,
#                       epoch, n_epochs):
#     # Print performance
...
#     use_dropout_feature=use_dropout_feature,
...
# # Define number of epochs and output directory
# n_epochs = 20
...
# # Train the model
# train_model(model, xtrain, ytrain, xvalid, yvalid, criterion, optimizer, device, n_epochs, output_dir)

def train(model, Xtrain, Ytrain, Xvalid, Yvalid, device, epochs=10, verbose=False, lr=0.001, batch_size=10):
...
# Train the model
train(model, Xtrain, Ytrain, Xvalid, Yvalid, epochs=10, verbose=True, lr=0.001, batch_size=10, device=device)